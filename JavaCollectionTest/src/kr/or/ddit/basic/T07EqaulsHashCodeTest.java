...
public class T07EqaulsHashCodeTest { // hashCode는 hashCode를 얻어내기 위한 메서드
	 ...
	 * 메서드
	 *
	 * equals()와 hashCode()에 관한 규칙 1. 두 객체가 같으면 반드시 hashCode를 가져야 한다. 2. 두 객체가 같으면
	 * equals()메서드를 호출했을 때 true값은 반환해야 한다. 즉, 객체a, b가 같다면 a.equals(b)와 b.equals)a) 둘
	 * 다 treu이어야 한다. 3. 두 객체의 hashCode가 같다고 해서 반드시 같은 객체는 아니다. 하지만 하지만 두 객체가 같으면 반드시
	 * hashCode가 같아야 한다 4.equals()를 override하면 반드시 hashCode()도 override 해야 함 5.
	 * hashCode() 기본적으로 Heap 메모리에 있는 각 객체에 대한 메모리 주소 매핑 정보를 기반으로 정수값 반환함 그러므로 클래스에서
	 * hashCode() 메서드를 override하지 않으면 절대로 두 객체가 같은 해시값을 반환하지 않는다.
	 * 
	 * - hashCode()에서 사용하는 '해싱알고리즘'에서 서로 다른 객체에 대하여 같은 hashCode값을 만들어낼 수 있다. 그래서 객체가
	 * 같지 않더라도 hashCode 값이 같을 수 있다.
	 */
