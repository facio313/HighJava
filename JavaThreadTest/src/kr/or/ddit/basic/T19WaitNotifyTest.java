...
/*
 wait() => 동기화 영역에서 락을 풀고 Wait-Set영역(공유객체별 존재)으로 이동시킨다. => 다른 스레드에게 진입할 기회를 주는 것
...
class WorkObject {
	public synchronized void methodA() {
		...
		notify(); // 누군가를 깨워주고
		// notifyAll()로 하면 다 깨워짐

		try {
			System.out.println(Thread.currentThread().getName() + " wait() 호출");
			wait(1000); // 락을 풀며 나는 대기실로 들어감, () 안에 값을 넣으면 마지막에 notify를 하지 않아도 B를 부름
