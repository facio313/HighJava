...
public class T11DisplayCharacterTest {
	/*
	 * 3개(명)의 스레드가 각각 알파벳 대문자를 출력하는데 출력을 긑낸 순서대로 결과를 나타내는 프로그램 작성하기
	 */

	static int currRank = 1; // 현재의 순위정보 , 제일 먼저 접근하는 놈이 1등, 그 다음 놈 ++.
...
class DisplayCharacter extends Thread implements Comparable<DisplayCharacter> {
	...
	public void run() {
		...
		setRank(T11DisplayCharacterTest.currRank++); // 앞에다 ++ 붙이면 2로 시작함. ++부터 계산하고 연산함
	}
