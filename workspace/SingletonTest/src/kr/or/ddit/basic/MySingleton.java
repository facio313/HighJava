...
/*
 *  싱글턴 패턴 => 객체(인스턴스)를 한 개만 만들어지도록 하는 프로그래밍 방법
 *  
 ...
 *  (이 메서드의 접근 제어자는 static, 이름은 보통 getInstacne()로 지정한다.)
 */
	...
	private MySingleton() {
		...
		// 다른 싱글톤을 가져오는 경우!(Controller - Service - DAO에서)
		// dao = Dao.getInstance();
		// --> static 메서드이기 때문에 클래스.메서드()하는 것 ex)Math.PI, Integer.parseInt() 등등

	}
	
	public static MySingleton getInstance() {
		if (single == null) { // null이면 객체가 한 번도 만들어지지 않은 것
