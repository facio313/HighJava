...
public class T05ServletCookieTest extends HttpServlet {
	...
	/*
	  쿠키 정보를 다루기 위한 예제
	  (웹 서버와 브라우저는 애플리케이션에서 사용 중인 필요한 값을 쿠키를 이용하여 상태 정보를 유지함)
	  쿠키 : 작은 정보 덩어리들(접속자의 설정, 과거 이용내역 등에 대한 일부 데이터)
	  사용자 정보를 브라우저에 저장하는 기술
	  ...
	  1. 구성요소
	   ...
	   - 경로 : 쿠키를 공유할 기준 경로(도메인 이후 부분으로 디렉토리 수준)
	   	=> 지정하지 않으면 실행한 URL의 경로 부분으로 지정됨
	   
	  2. 동작방식
	   - 쿠키 생성 단계 : 생성한 쿠키를 응답 데이터의 헤더에 저장하여 웹브라우저에 저장한다.
	   - 쿠키 저장 단계 : 브라우저는 응답 데이터에 포함된 쿠키를 쿠키 저장소에 보관한다.
	   - 쿠키 전송 단계 : 브라우저는 저장한 쿠키를 요청이 있을 때마다 웹 서버에 전송한다.(삭제되기 전까지)
	   웹 서버는 브라우저가 전송한 쿠키를 사용해 필요한 작업을 수행한다.
	 */
...
//		readCookieExam(req, resp); // 쿠키 정보 읽기 예제
	...
	private void deleteCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		/*
		 ...
		 1. 사용 중인 쿠키 정보를 이용하여 쿠키객체를 생성한다.
		 
		 2. 쿠키 객체의 최대 지속 시간을 0으로 설정한다.
		 
		 3. 설정한 쿠키 객체를 응답 헤더에 추가하여 전송한다.
		...
		if (cookies != null) {
			...
			for (Cookie cookie : cookies) {
				if (cookie.getName().equals("userId")) {
					...
					out.print("삭제한 쿠키 : " + cookie.getName());
				}
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		// 쿠키 생성하기
		// 사용 불가 문자(공백, []()=,"/?@:;) => RFC2965
		// 쿠키값은 사용 불가 문자를 제외한 나머지 출력가능한 ASCII 문자만 사용 가능함
		// 이외의 값(예를 들면 한글) 사용 시, URLEncoder.encode() 사용하여 인코딩 처리를 해준다.
		Cookie userId = new Cookie("userId", req.getParameter("userId"));
		Cookie name = new Cookie("name", req.getParameter("name"));
		
		// 쿠키 최대 지속 시간 설정하기(초 단위)
		// 지정하지 않으면 웹 브라우저 종료 시 쿠키 정보는 삭제된다.
